...
data <- read.csv("2VINE-t1rare.txt", head=TRUE,sep="\t",row.names = 1)
data=data.matrix(t(data))
groups <- read.table("3design.txt",sep = "\t",header = F,colClasses = c("character"))
colnames(groups) <- c("V1","V2")


cbbPalette <- c("#F8766D","#C77CFF", "#00BFC4", "#7CAE00")
                #,"#F0E442","#0072B2","#D55E00","#CC79A7","#CC6666","#9999CC","#66CC99","#99999",                "#ADD1E5")

data <-vegdist(decostand(data, method = "hellinger"), method = "euclidean")
...
plotdata$Group <- factor(plotdata$Group,levels = c("C","WC","W","CW"))
...
test$Group <- factor(test$Group,levels = c("C","WC","W","CW"))
...
p2<-ggplot(plotdata, aes(PC1, PC2)) +
  geom_point(aes(fill=Group),size=12,pch = 21)+
...
p5
ggsave("4VINEC101-T1-pca-eu-nohell.pdf",p5,width = 15,height = 12)
